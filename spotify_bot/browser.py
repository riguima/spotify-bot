...
class Browser:
    ...
    def listen_playlist(self, url):
        ...
        songs = self.find_elements('div[data-testid="tracklist-row"]')
        for e, song in enumerate(songs):
            if e != 0:
                ActionChains(self.driver).double_click(song).perform()
    ...
    def listen_playlist_song(self, url, song_index):
        ...
        if song_index == 0:
            ...
            song = self.find_elements('div[data-testid="tracklist-row"]')[song_index]
            ActionChains(self.driver).double_click(song).perform()
        sleep(random.uniform(1.5, 2) * 60)
